...
if ($action === "create") {
  try {
    ...
    if (intval($count) > 0) {
      $VALIDATION->alert("danger", "อีเมลซ้ำในระบบ!", "/user");
    }

    $password_default = $USER->password_default();
    $hash_password = password_hash($password_default, PASSWORD_DEFAULT);

    $USER->login_add([$email, $hash_password]);
    $login = $USER->last_insert_id();
    $USER->user_add([$login, $firstname, $lastname, $contact]);
...
if ($action === "change") {
  try {
    ...
    $USER->password_change([$hash_password, $uuid]);
