...
class AssetDepartment
{
  ...
  public function department_data()
  {
    ...
    foreach ($result as $row) {
      $action = "<a href='/asset/department/edit/{$row['uuid']}' class='badge badge-{$row['status_color']} font-weight-light'>{$row['status_name']}</a> <a href='javascript:void(0)' class='badge badge-danger font-weight-light btn-delete' id='{$row['uuid']}'>ลบ</a>";
      $data[] = [
        $action,
        $row['name']
      ];
